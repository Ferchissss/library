...
export function TimelineStats({ books }: TimelineStatsProps) {
  ...
  const calculateTaskPosition = (book: TimelineBook) => {
    ...
    if (viewMode === "day") {
      ...
      widthPixels = durationDays * columnWidth
    } else if (viewMode === "week") {
      const firstWeekStart = new Date(columns[0].date)
      const startWeekIndex = Math.floor((startDate.getTime() - firstWeekStart.getTime()) / (1000 * 60 * 60 * 24 * 7))
      const endWeekIndex = Math.floor((endDate.getTime() - firstWeekStart.getTime()) / (1000 * 60 * 60 * 24 * 7))
      
      const startWeekDate = new Date(firstWeekStart)
      startWeekDate.setDate(startWeekDate.getDate() + startWeekIndex * 7)
      const daysIntoStartWeek = Math.floor((startDate.getTime() - startWeekDate.getTime()) / (1000 * 60 * 60 * 24))
      
      const endWeekDate = new Date(firstWeekStart)
      endWeekDate.setDate(endWeekDate.getDate() + endWeekIndex * 7)
      const daysIntoEndWeek = Math.floor((endDate.getTime() - endWeekDate.getTime()) / (1000 * 60 * 60 * 24))
      
      leftPosition = startWeekIndex * columnWidth + (daysIntoStartWeek / 7) * columnWidth
      const endPosition = endWeekIndex * columnWidth + (daysIntoEndWeek / 7) * columnWidth
      widthPixels = endPosition - leftPosition
    } else if (viewMode === "month") {
      const startMonthIndex = (startDate.getFullYear() - minDate.getFullYear()) * 12 + (startDate.getMonth() - minDate.getMonth())
      const endMonthIndex = (endDate.getFullYear() - minDate.getFullYear()) * 12 + (endDate.getMonth() - minDate.getMonth())
      
      const daysIntoStartMonth = startDate.getDate() - 1
      const daysInStartMonth = new Date(startDate.getFullYear(), startDate.getMonth() + 1, 0).getDate()
      
      const daysIntoEndMonth = endDate.getDate()
      const daysInEndMonth = new Date(endDate.getFullYear(), endDate.getMonth() + 1, 0).getDate()
      
      leftPosition = startMonthIndex * columnWidth + (daysIntoStartMonth / daysInStartMonth) * columnWidth
      const endPosition = endMonthIndex * columnWidth + (daysIntoEndMonth / daysInEndMonth) * columnWidth
      widthPixels = endPosition - leftPosition
    } else if (viewMode === "year") {
      const startYearIndex = startDate.getFullYear() - minDate.getFullYear()
      const endYearIndex = endDate.getFullYear() - minDate.getFullYear()
      
      const startOfStartYear = new Date(startDate.getFullYear(), 0, 1)
      const startDayOfYear = Math.floor((startDate.getTime() - startOfStartYear.getTime()) / (1000 * 60 * 60 * 24))
      const daysInStartYear = 365 + (new Date(startDate.getFullYear(), 1, 29).getDate() === 29 ? 1 : 0)
      
      const startOfEndYear = new Date(endDate.getFullYear(), 0, 1)
      const endDayOfYear = Math.floor((endDate.getTime() - startOfEndYear.getTime()) / (1000 * 60 * 60 * 24))
      const daysInEndYear = 365 + (new Date(endDate.getFullYear(), 1, 29).getDate() === 29 ? 1 : 0)
      
      leftPosition = startYearIndex * columnWidth + (startDayOfYear / daysInStartYear) * columnWidth
      const endPosition = endYearIndex * columnWidth + (endDayOfYear / daysInEndYear) * columnWidth
      widthPixels = endPosition - leftPosition
    }

    return { left: leftPosition, width: Math.max(widthPixels, 20) }
  }
            ...
            <div>
              ...
              <div className="flex items-center gap-1">
                <div className="flex gap-0.5">
                  {Array.from({ length: Math.min(selectedBook.rating, 5) }, (_, i) => (
                ...
                {selectedBook.rating > 5 && (
                  <>
                    ...
                    <div className="flex gap-0.5">
                      {Array.from({ length: selectedBook.rating - 5 }, (_, i) => (
                        <Star key={i + 5} className="h-5 w-5 fill-red-500 text-red-500" />
                      ))}
                    </div>
                  </>
                )}
