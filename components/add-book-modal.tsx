...
import { Sparkles, FileText, BookOpenCheck, Plus, X, BookOpen } from "lucide-react"
...
export function AddBookModal({ trigger, refreshData }: AddBookModalProps) {
  ...
  const parseBulkData = () => {
    ...
    if (lines.length >= 2) {
      ...
      // Verificar si hay géneros nuevos que no están en las opciones
      const newGenres = bulkGenres.filter((genre) => !genresOptions.some((g) => g.value === genre))

      if (newGenres.length > 0) {
        toast({
          title: "Géneros nuevos detectados",
          description: `Por favor selecciona manualmente los géneros: ${newGenres.join(", ")}`,
          variant: "default",
        })
      }
